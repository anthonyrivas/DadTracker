$(document).ready(function () {
    $('body').on('click', '.editButton', function (e) {
        ...
        typeContainer.attr("style", "padding:0;");
        ...
        const saveButton = $(`<button type="button" class='btn btn-success mr-3 saveButton' data-id=${id}>Save</button>`)
    ...
    $('body').on('click', '.saveButton', function (e) {
        const id = $(this).attr('data-id');
        const typeContainer = $('#cp-type-' + id);
        typeContainer.attr("style", "padding: .375rem 0;");
        const messageContainer = $('#cp-message-' + id);
        const buttonContainer = $('#cp-button-' + id);
        const type = $('#checkpointTypeEdit-' + id).val();
        const message = $('#checkpointValueEdit-' + id).val();
        const editButton = $(`<button type="button" class='btn btn-info editButton' data-id=${id}>Edit</button>`)
